...
",
        ...
        private async Task<List<ErrorRowDto>?> FnValidateA(bool isNew, JObject json)
        {
            ...
            #region 工作日期不可大於今天
            var workDate = _Json.GetFidStr(row, "_WorkDate");
            ...
            var isAddJob = (_Json.GetFidStr(row, "_IsAddJob") == "1");
            var id = _Json.GetFidStr(row, "Id");
            var workHours = Convert.ToDouble(_Json.GetFidStr(row, "_WorkHours"));
            if (!isAddJob && workHours > 0)
            {
                //計算工作時數
                var sumStr = await _Db.GetStrA($@"
select sum(WorkHours) from dbo.Issue
where OwnerId='{_Fun.UserId()}'
and WorkDate=@WorkDate 
and Id != @Id
", ["WorkDate", workDate, "Id", id]);

                var sumHours = _Str.IsEmpty(sumStr) ? 0 : Convert.ToDouble(sumStr);
                if (workHours + sumHours > 8)
                {
                    result.Add(new ErrorRowDto()
                    {
                        Fid = "WorkHours",
            ...
            var rptType = _Json.GetFidStr(row, "_RptType");

...
_me = {
  ...
  init: function() {
    var config = {
      ...
      columns: [
        ...
        //9:寄送問卷次數
        { data: "_Survey" },
        //10.空白(沒有問卷Id && 沒有回報人)/已填問卷(有問卷Id)/寄送問卷(有回報人)
        { data: "_fun" }
      ],
      columnDefs: [
        ...
        { targets: [11], render: function(data, type, full, meta) {
          return _Str.notEmpty(full.SurveyId) ? "\u662F" : full.IsFinish == 1 && _Str.notEmpty(full.RptUser) && (full.OwnerId == _Fun.userId || _Fun.userId == "Bruce") ? `<button type='button' class='btn btn-secondary' data-onclick='_me.onSurvey' data-args='${full.Id}'>\u5BC4\u554F\u5377</button>` : "";
        } },
  ...
  fnAfterOpenEdit: function(fun, json) {
    ...
    if (isFunU || fun == FunEstr.View) {
      ...
      _iSelect.set("OwnerId", _Fun.userId, eform);
      _iDate.set("WorkDate", _Date.uiToday(), eform);
  ...
  onSurvey: async function(issueId) {
    if (await _Tool.ansA("\u662F\u5426\u78BA\u5B9A\u5BC4\u9001\u554F\u5377?")) {
      await _Ajax.getStrA("SendSurvey", { issueId }, function(str) {
        var ok = str == "1";
  ...
  onKeepIssue: () => {
    _Tool.ans("\u662F\u5426\u78BA\u5B9A\u5EF6\u7E8C\u6B64\u7B46\u5DE5\u4F5C?", () => {
      ...
      _iDate.set("WorkDate", _Date.uiToday(), _me.eform0);

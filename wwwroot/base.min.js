// _src/tsBase/dto/BaseResDto.ts
var BaseResDto = class {
};

// _src/tsBase/enum/EditModeEstr.ts
var EditModeEstr = class {
};
/**
 * default mode
 */
...
// _src/tsBase/enum/InputTypeEstr.ts
var InputTypeEstr = class {
};
...
// _src/tsBase/enum/NodeTypeEstr.ts
var NodeTypeEstr = class {
  //normal node
  //Auto: 'A',	//auto
};
// 原始物件字面量中的常數值被轉換為 static readonly 屬性，以符合 class 語法
// 且保持 NodeTypeEstr.Start 這樣的外部存取方式。
NodeTypeEstr.Start = "S";
//startNode
NodeTypeEstr.End = "E";
//endNode
NodeTypeEstr.Node = "N";

// _src/tsBase/enum/UiItemTypeEstr.ts
var UiItemTypeEstr = class {
  //(暫不使用)只能放 box, group, input, checks, span
};
UiItemTypeEstr.Input = "I";
//輸入欄位(含唯讀欄位)
UiItemTypeEstr.Group = "G";
//分群文字
UiItemTypeEstr.Checks = "CS";
//check list, 行為類似 input, 不是容器
//static readonly Span = 'S';	//只能放 input
UiItemTypeEstr.RowBox = "R";
//2/3欄, 只能放 group, input, checks, span
UiItemTypeEstr.Table = "T";
//只能放 input
UiItemTypeEstr.TabPage = "TP";

// _src/tsBase/dto/AjaxDto.ts
var AjaxDto = class {
};

// _src/tsBase/dto/ChartDto.ts
var ChartDto = class {
};

// _src/tsBase/dto/EditDto.ts
var EditDto = class {
  /**
...
// _src/tsBase/dto/ErrorRowDto.ts
var ErrorRowDto = class {
};

// _src/tsBase/dto/IdStrDto.ts
var IdStrDto = class {
};

// _src/tsBase/dto/IdStrExtDto.ts
var IdStrExtDto = class {
};

// _src/tsBase/dto/ResultDto.ts
var ResultDto = class {
};

// _src/tsBase/dto/DragItemDto.ts
var DragItemDto = class {
};

// _src/tsBase/svc/_Leftmenu.ts
var _Leftmenu = class __Leftmenu {
  static init() {
    __Leftmenu.menu = $(".x-leftmenu");
  ...
  */
  static onToggleMenu() {
    __Leftmenu.menu.toggleClass("x-close");
  }
  static getMenuPath(me) {
    ...
    me.parents("li").each(function() {
      ...
      if (parentLink.length) {
        parents.unshift(parentLink.text().trim());
      }
    });
...
// _src/tsBase/svc/_Var.ts
var _Var = class __Var {
  ...
  static isEmpty(var1) {
    return var1 === void 0 || var1 === null || var1 === "";
  }
  static isStr(var1) {
    return typeof var1 === "string";
  }
  static notEmpty(var1) {
    return !__Var.isEmpty(var1);
  }
...
// _src/tsBase/svc/_Str.ts
var __Str = class __Str {
  /**
   * 前端儲存檔案
  ...
  static getMid(str, find1, find2) {
    ...
    return pos2 < 0 ? currentStr.substring(pos + find1.length) : currentStr.substring(pos + find1.length, pos2);
  }
...
// _src/tsBase/svc/_Prog.ts
var __Prog = class __Prog {
  //original path
  static init() {
    __Prog.me = $(".x-prog-path");
    if (__Prog.me.text() == "") {
      if (_Str.isEmpty(_Fun.data.progPath)) {
        ...
        __Prog.storePath(menuPath);
      }
      __Prog.initPath = _Fun.data.progPath;
      __Prog.me.text(__Prog.initPath);
    }
  }
  ...
  static setPath(fun, updName) {
    const name = fun == FunEstr.Create ? _BR.Create : fun == FunEstr.View ? _BR.View : fun != FunEstr.Update ? "??" : _Str.isEmpty(updName) ? _BR.Update : updName;
    __Prog.setFunName(name);
  }
  /**
   * set fun name
  ...
  static setFunName(name) {
    __Prog.me.text(__Prog.initPath + "-" + name);
  }
};
//filter: '.x-prog-path',
__Prog.me = null;
...
var _Pjax = class {
  ...
  static init(boxFt) {
    ...
    docu.on("pjax:success", function(event, data, status, xhr, opts) {
      ...
      if (json != null) {
        const errMsg = _Ajax2.resultToErrMsg(json);
        if (errMsg) {
...
// _src/tsBase/svc/_iDate.ts
var __iDate = class __iDate extends _iBase {
  /**
   ...
   */
  static setO(obj, value) {
    __iDate._boxSetDate(__iDate._objToBox(obj), value);
  }
  /**
   * set edit status
   * param obj {object} date input object
   ...
   */
  static init(box, fid) {
    const obj = _Str.isEmpty(fid) ? box.find(__iDate.BoxFilter) : _Obj.get(fid, box).closest(__iDate.BoxFilter);
    ...
    obj.datepicker({
      ...
      __iDate._boxGetInput($(this)).valid();
    });
  ...
  static onToggle() {
    ...
    __iDate._elmToBox(btn).datepicker("show");
  }
  //reset value
  static onReset() {
    const btn = _Fun.getMeElm();
    const box = __iDate._elmToBox(btn);
    const input = __iDate._boxGetInput(box);
    if (__iDate.getEditO(input)) {
      __iDate._boxSetDate(box, "");
    }
  }
  ...
  static _elmToBox(elm) {
    return __iDate._objToBox($(elm));
  }
  static _objToBox(obj) {
    return obj.closest(__iDate.BoxFilter);
  }
...
__iDate.BoxFilter = ".date";
var _iDate = __iDate;

// _src/tsBase/svc/_iSelect.ts
var _iSelect = class extends _iBase {
  static get(fid, box) {
  ...
  static changeParent(upFid, childFid, childId, action, isEdit) {
    ...
    _Ajax2.getJsonA(action, { parentId: thisId }, (rows) => {
...
// _src/tsBase/svc/_Date.ts
var _Date = class __Date {
  ...
  static dsToUiDate(ds) {
    return _Str.isEmpty(ds) ? "" : __Date.mmToUiDate(dayjs(ds));
  }
  static dtsToUiDate(dts) {
    return _Str.isEmpty(dts) ? "" : __Date.mmToUiDate(dayjs(dts));
  }
  static dtsToFormat(ds, format) {
    if (format !== void 0) {
      return _Str.isEmpty(ds) ? "" : dayjs(ds).format(format);
    }
    return _Str.isEmpty(ds) ? "" : __Date.mmToUiDate(dayjs(ds));
  }
  static dtsToUiDt(dts) {
    return _Str.isEmpty(dts) ? "" : __Date.mmToUiDt(dayjs(dts));
  }
  /**
  ...
  static dtsToUiDt2(dts) {
    return _Str.isEmpty(dts) ? "" : __Date.mmToUiDt2(dayjs(dts));
  }
  //get datetime value for compare
  static dtsToValue(dts) {
    return _Str.isEmpty(dts) ? 0 : dayjs(dts).valueOf();
  }
  static dtsToMoment(dts) {
    return _Str.isEmpty(dts) ? null : dayjs(dts);
  }
  /**
  ...
  static uiToMmDate(ds) {
    var date = _Str.isEmpty(ds) ? "" : dayjs(ds, _BR.MmUiDateFmt).format(_Fun.MmDateFmt);
    return date;
  }
  /**
   ...
   */
  static tsToUiDt(ts) {
    return ts == "" ? "" : dayjs(parseInt(ts) * 1e3).format(_BR.MmUiDtFmt);
  }
  /**
  ...
  static getMonthDiff(ds1, ds2) {
    return _Str.isEmpty(ds1) || _Str.isEmpty(ds2) ? 0 : __Date.getMonthDiffByDate(dayjs(ds1), dayjs(ds2));
  }
  /**
  ...
  static getMonthDiffByDate(dt1, dt2) {
    return (dt2.year() - dt1.year()) * 12 + dt2.month() - dt1.month() + 1;
  }
  /**
...
// _src/tsBase/svc/_iText.ts
var _iText = class extends _iBase {
  static mask(box) {
...
// _src/tsBase/svc/_iTextarea.ts
var _iTextarea = class extends _iBase {
  /*
      ...
      static setO(obj: any, value: any): void {
          //obj.html(value);
          obj.val(value);
      }
      */
...
// _src/tsBase/svc/_iCheck.ts
var __iCheck = class __iCheck extends _iBase {
  /**
  ...
  static isChecked(fid, form) {
    return __iCheck.isCheckedO(_Obj.get(fid, form));
  }
  /**
   * checkedO -> isCheckedO
  ...
  static getCheck0Values(form) {
    const ary = [];
    const item = _Obj.getByFt(__iCheck.fltCheckeds, form);
  ...
  static checkAll(form, status) {
    __iCheck.setO(form.find(__iCheck.fltCheckeds), status);
  }
};
/**
 * Check0Id -> FidCheck0
 ...
 */
__iCheck.fidCheck0 = "_check0";
/**
 * filter for get checked list objects, 初始代階段不可使用 _Input
 */
__iCheck.fltCheckeds = "[data-fid='_check0']:checked";
var _iCheck = __iCheck;
Object.assign(_iCheck, _iBase);

// _src/tsBase/svc/_iRadio.ts
var _iRadio = class extends _iBase {
  //=== get ===
   ...
   */
  static setEditO(obj, status) {
    obj.attr("disabled", !status ? "disabled" : null);
  }
  //for modal單選畫面
  ...
  */
  static render(fid, label, checked, value, editable, extClass, extProp) {
    const html = "<label class='xi-check {0}'>   <input type='radio'{1}>{2}   <span class='xi-rspan'></span></label>";
    label = label || "";
    extClass = extClass || "";
    extProp = extProp || "";
    let valStr = value || "";
    ...
    if (_Str.isEmpty(valStr))
      valStr = 1;
    extProp += " data-id='" + fid + "' name='" + fid + "' value='" + valStr + "'";
    if (checked)
      extProp += " checked";
...
// _src/tsBase/svc/_iDt.ts
var _iDt = class __iDt extends _iDate {
  //constant
  //BoxFilter: '.date',
  //=== get/set start ===
  static getO(obj) {
    const date = _iDate.getO(__iDt._boxGetDate(obj));
    return _Str.isEmpty(date) ? "" : date + " " + _iSelect.getO(__iDt._boxGetHour(obj)) + ":" + _iSelect.getO(__iDt._boxGetMin(obj));
  }
  /**
   * set input value
  ...
  static setO(obj, value) {
    ...
    _iDate.setO(__iDt._boxGetDate(obj), date);
    _iSelect.setO(__iDt._boxGetHour(obj), hour);
    _iSelect.setO(__iDt._boxGetMin(obj), min);
  }
  static setEditO(obj, status) {
    _iDate.setEditO(__iDt._boxGetDate(obj), status);
    _iSelect.setEditO(__iDt._boxGetHour(obj), status);
    _iSelect.setEditO(__iDt._boxGetMin(obj), status);
  }
...
// _src/tsBase/svc/_iFile.ts
var _iFile = class __iFile extends _iBase {
  //=== overwrite start ===
  /**
   * get border object
  ...
  static setO(obj, value) {
    obj.val(value);
    __iFile._elmToLink(obj).text(value);
  }
  //=== overwrite end ===
  /**
  ...
  static dataAddFile(data, fid, serverFid, box) {
    const obj = _Obj.get(fid, box);
    const file = __iFile.getUploadFile(__iFile._elmToFile(obj));
  ...
  static onOpenFile() {
    ...
    const file = __iFile._elmToFile(btn);
    file.focus().trigger("click");
  }
  //file: input element
  static onChangeFile() {
    const fileElm = _Fun.getMeElm();
    const obj = __iFile._elmToObj(fileElm);
    const fileObj = $(fileElm);
    const value = fileElm.value;
    if (_Str.isEmpty(value)) {
      __iFile.setO(obj, "");
      return;
    }
    ...
    __iFile.setO(obj, _File.getFileName(value));
  }
  static onDeleteFile() {
    const btn = _Fun.getMe();
    __iFile.setO(__iFile._elmToObj(btn), "");
  }
  //=== event end ===
  ...
  static _elmToFile(elm) {
    return __iFile._boxGetFile(__iFile._elmToBox(elm));
  }
  //get input object
  static _elmToObj(elm) {
    return __iFile._boxGetObj(__iFile._elmToBox(elm));
  }
  //get link object
  static _elmToLink(elm) {
    return __iFile._boxGetLink(__iFile._elmToBox(elm));
  }
  /**
   * box get link object
...
// _src/tsBase/svc/_iHtml.ts
var __iHtml = class __iHtml extends _iBase {
  static getO(obj) {
  ...
  static init(edit, prog, height) {
    edit.eform.find(__iHtml.Filter).each(function() {
      const upMe = $(this);
      upMe.data("prog", prog);
      upMe.summernote({
        height: height || 200,
          ...
          onImageUpload: function(files) {
            ...
            $.ajax({
              ...
              success: function(url) {
                ...
                image.src = url;
                me.summernote("insertNode", image);
              }
            });
          }
        }
  ...
  static setEdits(box, subFilter, status) {
    const item = box.find(__iHtml.Filter + subFilter);
    if (item.length > 0) {
...
//constant
__iHtml.Filter = "[data-type=html]";
var _iHtml = __iHtml;
Object.assign(_iHtml, _iBase);

// _src/tsBase/svc/_iRead.ts
var _iRead = class {
  //value by fid
  static get(fid, form) {
...
// _src/tsBase/svc/_Input.ts
var _Input = class __Input {
  static isRadio(ftype) {
    return ftype === InputTypeEstr.Radio;
  }
  //get input value
  static get(fid, box) {
    return __Input.getO(_Obj.get(fid, box), box);
  }
  /**
  ...
  static getO(obj, box, type) {
    ...
    switch (type) {
      ...
      default:
        return (_a = obj == null ? void 0 : obj.val) == null ? void 0 : _a.call(obj);
    }
  }
  static set(fid, value, box) {
    __Input.setO(_Obj.get(fid, box), value, box);
  }
  /**
  ...
  static setO(obj, value, box, type) {
    ...
    switch (type) {
      ...
      case InputTypeEstr.Html:
        ...
        break;
      case InputTypeEstr.Read:
        const format = (_a = obj == null ? void 0 : obj.data) == null ? void 0 : _a.call(obj, "format");
      ...
      default:
        (_b = obj == null ? void 0 : obj.val) == null ? void 0 : _b.call(obj, value);
        break;
    }
  }
  /**
  ...
  static getType(obj) {
    var _a, _b, _c, _d, _e;
    return (_e = (_a = obj == null ? void 0 : obj.attr) == null ? void 0 : _a.call(obj, "data-type")) != null ? _e : (_d = (_c = (_b = obj == null ? void 0 : obj.find) == null ? void 0 : _b.call(obj, "[data-type]")) == null ? void 0 : _c.attr) == null ? void 0 : _d.call(_c, "data-type");
  }
  /**
   * get object
   * param fid {string}
  ...
  static getObj(fid, box, ftype) {
    ftype = ftype || __Input.getType(_Obj.get(fid, box));
    return __Input.isRadio(ftype != null ? ftype : "") ? _iRadio.getObj(fid, box) : _Obj.get(fid, box);
  }
  /**
  ...
  static getFid(obj) {
    var _a;
    return (_a = obj == null ? void 0 : obj.data) == null ? void 0 : _a.call(obj, "fid");
  }
  /**
  ...
  static fidFilter(fid) {
    return _Str.isEmpty(fid) ? "[data-fid]" : `[data-fid='${fid}']`;
  }
...
// _src/tsBase/svc/_Obj.ts
var _Obj = class __Obj {
  static toAny(obj) {
    return obj;
  }
   ...
   */
  static get(fid, box) {
    return __Obj.getByFt(_Input.fidFilter(fid), box);
  }
  /**
   * getF -> getByFt
...
// _src/tsBase/svc/_Tool.ts
var __Tool = class __Tool {
  static init() {
    __Tool.xMsg = $("#xMsg");
    __Tool.xAns = $("#xAns");
    __Tool.xAnsA = $("#xAnsA");
    __Tool.xAlert = $(".x-alert");
    __Tool.xArea = $(".x-area");
    __Tool.xImage = $(".x-image");
    __Tool.xWork = $(".x-work");
  }
  /**
   * show message box
  ...
  static msg(msg, fnClose) {
    const box = __Tool.xMsg;
    box.find(__Tool.FltMsgText).html(msg);
    _Modal.show(box);
    __Tool._fnOnMsgClose = fnClose;
  }
  /**
   * show confirmation 
   * @param msg
  ...
  static ans(msg, fnYes, fnNo) {
    const box = __Tool.xAns;
    box.find(__Tool.FltMsgText).html(msg);
    _Modal.show(box);
    __Tool._fnOnAnsYes = fnYes;
    __Tool._fnOnAnsNo = fnNo === void 0 ? null : fnNo;
  }
  /**
  ...
  static async ansA(msg) {
    const box = __Tool.xAnsA;
    box.find(__Tool.FltMsgText).html(msg);
    _Modal.show(box);
    return new Promise((resolve) => {
      __Tool.ansStatus = false;
      __Tool.fnResolve = resolve;
    });
  }
  // called by ansA yes/no onclick event
  static onAnsA(value) {
    if (__Tool.ansStatus) return;
    __Tool.ansStatus = true;
    _Modal.hide(__Tool.xAnsA);
    if (__Tool.fnResolve) {
      __Tool.fnResolve(value == 1);
    }
  }
  /**
  ...
  static alert(msg, color) {
    const box = __Tool.xAlert;
    box.find(__Tool.FltMsgText).text(msg);
  ...
  // ??show waiting
  static showWait() {
    _Obj.show(__Tool.xAlert);
  }
  // ??
  static hideWait() {
    _Obj.hide(__Tool.xAlert);
  }
  /**
  ...
  static showArea(title, value, isEdit, fnOk) {
    ...
    if (isEdit) {
      __Tool._fnOnAreaYes = fnOk;
    }
    _Modal.show(box);
  }
  static onAreaYes() {
    const box = __Tool.xArea;
    if (__Tool._fnOnAreaYes) {
      _Modal.hide(box);
      const value = box.find("textarea").val();
      __Tool._fnOnAreaYes(value);
    }
  }
  /**
   * show image modal
   ...
   */
  static onAnsYes() {
    if (__Tool._fnOnAnsYes) {
      _Modal.hide(__Tool.xAns);
      __Tool._fnOnAnsYes();
    }
  }
  static onAnsNo() {
    if (__Tool._fnOnAnsNo) {
      __Tool._fnOnAnsNo();
    }
    _Modal.hide(__Tool.xAns);
  }
  static onMsgClose() {
    if (__Tool._fnOnMsgClose) {
      __Tool._fnOnMsgClose();
    }
    _Modal.hide(__Tool.xMsg);
  }
};
// constant
// msg text
__Tool.FltMsgText = ".xd-msg";
// variables for ansA
__Tool.ansStatus = false;
__Tool.fnResolve = null;
__Tool._fnOnMsgClose = null;
__Tool._fnOnAnsYes = null;
__Tool._fnOnAnsNo = null;
__Tool._fnOnAreaYes = null;
var _Tool = __Tool;

// _src/tsBase/svc/_Fun.ts
var __Fun = class __Fun {
  /**
   * initial
   * param {string} locale
   ...
   */
  static init(locale) {
    __Fun.jwtToken = localStorage.getItem("_jwtToken") || "";
    localStorage.removeItem("_jwtToken");
    __Fun.locale = locale;
    _Leftmenu.init();
    _Pjax.init(".x-main-right");
    _Tool.init();
    __Fun.setLocaleA(locale);
    _Date.setLocale(locale);
    var body = $("body");
    __Fun.setEvent(body, "click");
    __Fun.setEvent(body, "change");
    $.ajaxSetup({
      headers: {
  ...
  static getMeValue() {
    return _Input.getO($(__Fun.nowDom));
  }
  /**
...
__Fun.MmDateFmt = "YYYY/MM/DD";
__Fun.MmDtFmt = "YYYY/MM/DD HH:mm:ss";
__Fun.FidErrorMsg = "_ErrorMsg";
__Fun.PreBrError = "B:";
__Fun.CssFlag = "x-flag";
__Fun.HideRwd = "x-hide-rwd";
// #endregion
// variables
__Fun.userId = "";
__Fun.locale = "zh-TW";
__Fun.maxFileSize = 50971520;
//upload file limit(50M)
__Fun.isRwd = false;
__Fun.pageRows = 10;
//must be 10,20(not 25),50,100
__Fun.nowDom = "";
//now dom event element
__Fun.lengthMenu = [10, 20, 50, 100];
__Fun.jwtToken = "";
//for JWT, 登入後自行設定內容
// mid variables
__Fun.data = {};
...
// _src/tsBase/svc/_Jwt.ts
var _Jwt = class __Jwt {
  /**
   * get header json object for jwt
   */
  static jsonAddJwtHeader(json) {
    if (_Fun.jwtToken) {
      json.headers = __Jwt.getJwtAuth();
    }
  }
  static getJwtAuth() {
    return {
      "Authorization": __Jwt.getJwtBearer()
    };
  }
  static getJwtBearer() {
...
// _src/tsBase/svc/_Ajax.ts
var _Ajax2 = class __Ajax {
  /** * ajax return json
  ...
  static async getJsonA(url, data, fnOk, block) {
    const json = {
      ...
      data,
      dataType: "json"
      //return type: ContentType,JsonResult
    };
    return await __Ajax._rpcA(json, fnOk, block);
  }
  static async getJsonsA(url, data, fnOk, block) {
    const json = {
      url,
      type: "POST",
      data,
  ...
  static async getJsonByFdA(url, data, fnOk, block) {
    const json = {
      ...
      data,
      dataType: "json",
      //return type, TODO: pending test
      cache: false,
      contentType: false,
      //false!! input type, default 'application/x-www-form-urlencoded; charset=UTF-8'
      processData: false
      //false!! (jQuery only) if true it will convert input data to string, then get error !!
    };
    return await __Ajax._rpcA(json, fnOk, block);
  }
  /**
  ...
  static async getStrA(url, data, fnOk, block) {
    const json = {
      ...
      data,
      dataType: "text"
      //backend return text(ContentResult with text)
    };
    return await __Ajax._rpcA(json, fnOk, block);
  }
  ...
  static async _rpcA(json, fnOk, block) {
    ...
    try {
      ...
      const errMsg = __Ajax.resultToErrMsg(result);
      if (_Str.notEmpty(errMsg)) {
        ...
        for (let i = 0; i < result.ErrorRows.length; i++) {
          ...
          const edit = row.EditNo == 0 ? _me.edit0 : _me.crudE.getEditByNo(row.EditNo);
  ...
  static resultToErrMsg(result) {
    return result && result[_Fun.FidErrorMsg] ? __Ajax.strToErrMsg(result[_Fun.FidErrorMsg]) : "";
  }
  /**
  ...
  static strToErrMsg(str) {
    if (_Str.isEmpty(str))
      return "";
    if (!__Ajax._isBrError(str))
      return str;
    const fid = str.substring(2);
    return _BR[fid] ? _BR[fid] : _Str.format("_Ajax.strToErrMsg() failed, no BR Fid={0}", fid);
  }
};

// _src/tsBase/svc/_Array.ts
var _Array2 = class __Array {
  /**
   * find array
  ...
  static notEmpty(ary) {
    return !__Array.isEmpty(ary);
  }
};

// _src/tsBase/svc/_Error.ts
var _Error = class {
  static log(msg) {
    console.log(msg);
  }
...
// _src/tsBase/svc/_Assert.ts
var _Assert = class __Assert {
  static echo(msg) {
  ...
  static inArray(value, ary) {
    ...
    if (!find)
      __Assert.echo("inArray failed: " + value);
  }
};

// _src/tsBase/svc/_Browser.ts
var _Browser = class {
...
// _src/tsBase/svc/_Json.ts
var _Json = class __Json {
  /**
  ...
  */
  ...
  static toStr(json) {
    return __Json.isEmpty(json) ? "" : JSON.stringify(json);
  }
  static isEmpty(json) {
    return json == null || $.isEmptyObject(json);
  }
  static notEmpty(json) {
    return !__Json.isEmpty(json);
  }
  static fidIsEmpty(json, fid) {
    return __Json.isEmpty(json) ? true : _Var.isEmpty(json[fid]);
  }
  static fidNotEmpty(json, fid) {
    return !__Json.fidIsEmpty(json, fid);
  }
  ...
  static removeNull(obj, level) {
    ...
    $.each(obj, function(key, value) {
      if (value === null) {
        delete obj[key];
      } else if (__Json.isKeyValue(value)) {
        __Json.removeNull(value, currentLevel + 1);
        ...
        $.each(value, function(k2, v2) {
          __Json.removeNull(v2, currentLevel + 1);
          if (__Json.isEmpty(v2))
            value[k2] = null;
        });
        let isEmpty = true;
        for (let i = len - 1; i >= 0; i--) {
          if (!__Json.isEmpty(value[i])) {
            isEmpty = false;
          } else if (isEmpty) {
            delete value[i];
          } else {
    ...
    if (__Json.isEmpty(obj)) {
      obj = null;
    }
  }
};

// _src/tsBase/svc/_Chart.ts
var __Chart = class __Chart {
  /**
   * show chart
  ...
  static line(canvasElm, dto) {
    return __Chart._show("line", canvasElm, dto, false);
  }
  //水平條狀圖
  static hbar(canvasElm, dto) {
    ...
    return __Chart._show("hbar", canvasElm, dto, false);
  }
  //圓餅圖
  static pie(canvasElm, dto) {
    return __Chart._show("pie", canvasElm, dto, null, true);
  }
  //甜甜圈
  static doughnut(canvasElm, dto) {
    return __Chart._show("doughnut", canvasElm, dto, null, true);
  }
  //多個線形圖
  static groupLine(canvasElm, dto) {
    return __Chart._show("line", canvasElm, dto);
  }
  //多個線形圖
  static groupBar(canvasElm, dto) {
    return __Chart._show("bar", canvasElm, dto);
  }
  /**
...
// _src/tsBase/svc/_Code.ts
var _Code = class __Code {
  //add empty option
  static addEmptyOpt(rows = []) {
    rows.unshift({ Id: "", Str: _BR.PlsSelect });
    return rows;
  }
  /**
   * filter json array
  ...
  static filterRows(rows = [], value, addEmpty = true) {
    let result = rows.filter((row) => row.Ext === value).map((row) => ({ Id: row.Id, Str: row.Str }));
    return addEmpty ? __Code.addEmptyOpt(result) : result;
  }
};

// _src/tsBase/svc/_Dom.ts
var _Dom = class {
  //傳回字串, 不會自動轉型
...
// _src/tsBase/svc/_Form.ts
var _Form = class __Form {
  /**
  ...
  static toRowStr(form) {
    return JSON.stringify(__Form.toRow(form));
  }
  /**
...
var EditOne = class {
  constructor(kid, eformId, childs) {
    ...
    this.dataJson = null;
    this.systemError = "";
    var error = this.eform.length != 1 ? "EditOne.js input eformId is wrong. (" + eformId + ")" : _Obj.get(this.kid, this.eform) == null ? "EditOne.js input kid is wrong. (" + this.kid + ")" : "";
    if (error != "") {
...
// _src/tsBase/svc/_Edit.ts
var __Edit = class __Edit {
  ...
  static jsonGetRows(json) {
    return json == null || json[__Edit.Rows] == null ? null : json[__Edit.Rows];
  }
  static jsonGetRows0(json) {
    const rows = __Edit.jsonGetRows(json);
  ...
  static getChildJson(upJson, childIdx) {
    const childs = __Edit.Childs;
    return upJson == null || upJson[childs] == null || upJson[childs].length <= childIdx ? null : upJson[childs][childIdx];
  }
  //upJson get child rows
  static getChildRows(upJson, childIdx) {
    const child = __Edit.getChildJson(upJson, childIdx);
    return __Edit.jsonGetRows(child);
  }
  static getChildRows0(upJson, childIdx) {
    const rows = __Edit.getChildRows(upJson, childIdx);
    return rows == null || rows.length == 0 ? null : rows[0];
  }
  /**
   ...
   */
  static getOld(obj) {
    return obj.data(__Edit.DataOld);
  }
  /**
   * set old value
   * param obj {object} input JQuery object
  ...
  static isNewRow(row, kid) {
    return __Edit.isNewKey(row[kid]);
  }
  /**
  ...
  static isNewBox(box, kid) {
    return __Edit.isNewKey(_Input.get(kid, box));
  }
  /**
  ...
  static loadRow(edit, box, row) {
    ...
    for (let i = 0; i < edit.fidTypeLen; i = i + 2) {
      ...
      const obj = _Obj.get(fid, box);
      obj.data(__Edit.DataOld, row[fid]);
    }
  }
  /**
  ...
  static getUpdRow(edit, box) {
    ...
    if (__Edit.isNewRow(row, edit.kid)) {
      for (let j = 0; j < edit.fidTypes.length; j = j + 2) {
        ...
        if (_Var.notEmpty(value)) {
          if ((ftype === "date" || ftype === "dt") && _Date.dtsToValue(value) === _Date.dtsToValue(void 0))
            continue;
          result[fid] = value;
        }
      }
    ...
    for (let j = 0; j < edit.fidTypes.length; j = j + 2) {
      ...
      ftype = edit.fidTypes[j + 1];
      obj = _Input.getObj(fid, box, ftype);
      old = obj ? _Obj.getData(obj, __Edit.DataOld) : "";
      value = row[fid];
      if (value != old) {
        if ((ftype === "date" || ftype === "dt") && _Date.dtsToValue(value) === _Date.dtsToValue(old))
          continue;
        result[fid] = value;
        diff = true;
      }
  ...
  static async viewFileA(table, fid, elm, key) {
    ...
    await _Ajax2.getFileA("ViewFile", data, elm);
  }
  /**
...
__Edit.Rows = "_rows";
__Edit.Childs = "_childs";
//注意: 同時用在json資料、EditOne/EditMany(表達下層物件)
__Edit.Deletes = "_deletes";
__Edit.DataFkeyFid = "_fkeyfid";
//data field for fkey fid, lowercase
//server side fid for file input collection, must pre '_'
//key-value of file serverFid vs row key
__Edit.FileJson = "_fileJson";
//data property name for keep old value
__Edit.DataOld = "_old";
var _Edit = __Edit;

// _src/tsBase/svc/_Flow.ts
var _Flow = class {
...
// _src/tsBase/svc/_Group.ts
var _Group = class {
  static toggle() {
    _Fun.getMe().parent(".x-group").next().slideToggle();
  }
};

// _src/tsBase/svc/_Helper.ts
var _Helper = class {
  /**
   * ??
   */
...
// _src/tsBase/svc/_Html.ts
var _Html = class __Html {
  //load css theme
  ...
  static encode(value) {
    var _a;
    return (_a = $("<div/>").text(value).html()) != null ? _a : "";
  }
  static decode(value) {
    var _a;
    return (_a = $("<div/>").html(value).text()) != null ? _a : "";
  }
  //?? 更新html欄位內容, 讀取 text()
  static update(id, box) {
    const filter = "#" + id;
    const obj = box === void 0 ? $(filter) : box.find(filter);
    obj.summernote("code", obj.text());
  }
  //??
  static updates(ids, box) {
    for (let i = 0; i < ids.length; i++) {
      __Html.update(ids[i], box);
    }
  }
};

// _src/tsBase/svc/_iColor.ts
var _iColor = class __iColor {
  /*
  static init(): void {
      $('.x-color').colorpicker({
          //component: true,
  ...
  */
  static get(fid, form) {
    return __iColor.getO(_Obj.get(fid, form));
  }
  //value by filter
  static getF(filter, form) {
    return __iColor.getO(_Obj.getByFt(filter, form));
  }
  //value by object
  static getO(obj) {
    return __iColor.rgbToHex(obj.find("i").css("background-color") || "");
  }
...
// _src/tsBase/svc/_iNum.ts
var _iNum = class extends _iBase {
  /*
...
// _src/tsBase/svc/_Log.ts
var __Log = class __Log {
  //目前時間
  ...
  static logTimeInit(name) {
    __Log._start = /* @__PURE__ */ new Date();
    __Log._now = __Log._start;
    if (name)
      console.log(name);
  }
  /**
  ...
  static logTime(name) {
    ...
    const msg = name + ":" + (now - __Log._now) + "/" + (now - __Log._start);
    console.log(msg);
    __Log._now = /* @__PURE__ */ new Date();
  }
};
/**
...
// _src/tsBase/svc/_Num.ts
var _Num = class __Num {
  //是否為數字而且大於(等於)0
  //zeor: 可否為0
...
// _src/tsBase/svc/_Qrcode.ts
var _Qrcode = class __Qrcode {
  static set(id, box, url, width) {
    return __Qrcode.setO(_Obj.getById(id, box), url, width);
  }
...
var _Switch = class {
  ...
  static getText(yes, no, width, status, inline, fid, cls) {
    ...
    cls = cls ? ` ${cls}` : "";
    const html = '<label class="switch{5}" style="width:{2}px;"><input{3} class="switch-input{4}" type="checkbox" /><span class="switch-label" data-on="{0}" data-off="{1}"></span><span class="switch-handle"></span></label>';
    return _Str.format(html, yes, no, width, attr, cls, inline2);
  }
};

// _src/tsBase/svc/_Tab.ts
var _Tab = class {
  static moveLeft(obj) {
    obj.insertBefore(obj.prev());
  }
...
// _src/tsBase/svc/_Valid.ts
var _Valid = class __Valid {
  /**
  ...
  static init(form) {
    ...
    const config = {
      ...
      errorPlacement: function(error, elm) {
        error.insertAfter(__Valid._getBox($(elm)));
        return false;
      },
      //顯示validation錯誤
      highlight: function(elm, errorClass, validClass) {
        const me = $(elm);
        const box = __Valid._getBox(me);
        box.removeClass(validClass).addClass(errorClass);
        const errObj = __Valid._getError(me);
        if (errObj != null) {
          _Obj.show(errObj);
        }
        return false;
  ...
  static _getError(obj) {
    const error = __Valid._getBox(obj).next();
    return error.length == 1 && error.hasClass("error") && error.is("span") ? error : null;
  }
};

// _src/tsBase/svc/CrudE.ts
var CrudE2 = class {
  constructor(edits) {
    this._nowFun = "";
    this._Edits = edits;
  ...
  mEditGetDivEdit() {
    return this._multiEdit ? this._Edits[this._nowEditNo].divEdit : _me.divEdit;
  }
  ...
  _loadJson2(edit, json) {
    ...
    if (_Edit.isEditOne(edit)) {
      edit.loadRow(_Array2.isEmpty(rows) ? null : rows[0]);
      edit.dataJson = json;
    } else {
  ...
  _getUpdJson2(edit, key, levelStr, formData, fileJson, dataJson) {
    ...
    const json = isOne ? edit.getUpdRow(key) : edit.getUpdJsonBySys(key);
  ...
  async _getJsonAndEditA(fun, key) {
    ...
    const act = fun == FunEstr.Update ? "GetUpdJson" : fun == FunEstr.Create ? "GetSignJson" : "GetViewJson";
    await _Ajax2.getJsonA(act, data, function(json) {
  ...
  async onSaveA() {
    ...
    if (this._hasFile()) {
      ...
      await _Ajax2.getJsonByFdA(action, data, function(result) {
        me.afterSave(result);
      });
    } else {
      ...
      await _Ajax2.getJsonA(action, data, function(result) {
        me.afterSave(result);
      });
    }
  }
...
var Datatable = class {
  constructor(selector, url, dtConfig, findJson, fnOk, tbarHtml, fnAfterFind) {
    ...
    var config = {
      ...
      //set page rows
      info: true,
      //show page info, include now page, total pages
      sorting: [],
      //default not sorting, or datatable will sort by first column !!
...
`,
      ...
      ajax: {
        ...
        */
        ...
        data: function(arg) {
          ...
          arg.findJson = _Json.toStr(this.findJson);
        ...
        dataSrc: function(result) {
          ...
          this._keepStart = false;
          var errMsg = _Ajax2.resultToErrMsg(result);
          if (errMsg) {
            _Tool.msg(errMsg);
    ...
    if (dtConfig) {
      if (_Var.notEmpty(dtConfig.columnDefs)) {
        ...
        colDefs[colDefs.length] = _Fun.dtColDef;
      }
  ...
  reload() {
    ...
    this._nowShowOk = false;
    this.find(this.findJson);
  }
};

// _src/tsBase/svc/CrudR.ts
var CrudR = class {
  ...
  dtCheck0(value, editable) {
    if (_Str.isEmpty(value)) value = 1;
    var attr = "data-fid='" + _iCheck.fidCheck0 + "' data-value='" + value + "'";
    if (editable === false) attr += " readonly";
    return "<label class='xi-check x-no-label'>   <input " + attr + " type='checkbox'>   <span class='xi-cspan'></span></label>";
  }
  //??
  ...
  dtStatusName(value) {
    return value == "1" ? "<span>" + _BR.StatusYes + "</span>" : '<span class="text-danger">' + _BR.StatusNo + "</span>";
  }
  dtYesEmpty(value) {
  ...
  //顯示紅色文字 by status
  dtRed(text, status) {
    return status ? '<span class="text-danger">' + text + "</span>" : "<span>" + text + "</span>";
  }
  /**
  ...
  */
  ...
  async onCreate() {
    var me = this;
    if (this.hasDraft) {
      await _Ajax2.getJsonA("GetDraftJson", { key: "" }, function(json) {
  ...
  */
  ...
  async onSetStatusA(me, key) {
    ...
    await _Ajax2.getStrA("SetStatus", { key, status }, function(msg) {
  ...
  async onDeleteA(key, rowName) {
    ...
    _Tool.ans(_BR.SureDeleteRow + " (" + rowName + ")", async function() {
      await _Ajax2.getJsonA("Delete", { key }, function(msg) {
  ...
  async onDeleteRowsA(box) {
    ...
    _Tool.ans(_BR.SureDeleteSelected, async function() {
      await _Ajax2.getStrA("DeleteByKeys", { keys }, function(msg) {
        _Tool.alert(_BR.DeleteOk);
        me.dt.reload();
      });
    });
...
var EditMany3 = class {
  ...
  valid() {
    return this.fnValid ? this.fnValid() : this.hasEform ? this.eform.validTable(this.validator) : true;
  }
  getKey(rowBox) {
  ...
  _elmToRowBox(elm) {
    return this._checkRowFilter() ? $(elm).closest(this.rowFilter) : null;
  }
  idToRowBox(id) {
  ...
  getDeletes() {
    return this.deletedRows.length === 0 ? null : this.deletedRows.join();
  }
  onAddRow() {
    this.addRow();
  }
...
var FlowLine = class {
  constructor(flowView, json, fromNode, toNode) {
    ...
    this.path = this.svg.path("").attr("data-id", json.Id).addClass("xf-line");
    this.path2 = this.svg.path("").attr("data-id", json.Id).fill("none").stroke({ width: 10, color: "transparent" }).attr({ "pointer-events": "stroke", "cursor": "pointer" });
    this.labelElm = this.svg.text(json.Label).addClass("xf-line-text").font({ anchor: "middle" });
  ...
  render() {
    ...
    this.labelElm.center(
      (points[textStartAry].x + points[textStartAry + 1].x) / 2,
...
var FlowNode2 = class {
  constructor(flowView, json) {
    ...
    let cssClass = "";
    let nodeText = "";
    this.elm = this.svg.group().attr("data-id", json.Id);
    ...
    if (startEnd) {
      ...
      this.boxElm = this.elm.circle().addClass(cssClass);
      this.boxElm.attr("r", this.NodeRadius);
      this.nameElm = this.elm.text(nodeText).addClass(cssClass + "-text").attr({ "text-anchor": "middle", "dominant-baseline": "middle" });
    } else {
      nodeText = this.json.Name;
      cssClass = "xf-node";
      this.boxElm = this.elm.rect().addClass(cssClass).attr({
        "text-anchor": "middle",
        "dominant-baseline": "middle",
        "rx": this.NodeRx,
        "ry": this.NodeRx
      });
      this.nameElm = this.elm.text("").addClass(cssClass + "-text");
      this.setName(nodeText, false);
    }
    this.elm.move(this.json.PosX, this.json.PosY);
    if (nodeType != NodeTypeEstr.End) {
      this.pinElm = this.elm.rect(this.PinWidth, this.PinWidth).addClass("xf-pin");
      this._setPinPos();
    }
    this._setEvent();
  }
  ...
  _setEventPin() {
    ...
    this.pinElm.draggable().on(MouseEstr.DragStart, (event) => {
      ...
      tempLine = me.svg.line(startX, startY, startX, startY).addClass("xf-line off");
      ...
      if (isFinite(endX) && isFinite(endY)) {
        ...
        let overDom = document.elementsFromPoint(viewPortX, viewPortY).find((dom) => dom != fromDom && (dom.classList.contains("xf-node") || dom.classList.contains("xf-end")));
        if (overDom) {
...
var FlowView2 = class {
  ...
  addNode(json) {
    let node = new FlowNode2(this, json);
...
var FlowMany = class {
  constructor(areaId, mNode, mLine) {
    ...
    const flowView = new FlowView2(areaId);
  ...
  fnShowMenu(event, isNode, flowItem) {
    ...
    const canEdit = isNode ? this.isEdit && flowItem.getNodeType() == NodeTypeEstr.Node : this.isEdit;
    const css = "off";
    ...
    menu.finish().removeClass("d-none").css({
      position: "fixed",
  ...
  _getCondStr() {
    ...
    this.tbodyLineCond.find("tr").each(function(idx) {
      const tr = $(this);
      const str = (idx == 0 ? "" : _iSelect.get("AndOr", tr)) + _iText.get("Fid", tr) + me.ColSep + _iSelect.get("Op", tr) + me.ColSep + _iText.get("Value", tr);
      condStr += str;
    });
    return condStr;
  }
...
var Page = class {
  ...
  _init(pageStr, onFind) {
    ...
    if (this.showMenu) {
      ...
      const info = cols[1].replace("_Start", start.toString()).replace("_End", (end <= arg.filterRows ? end : arg.filterRows).toString()).replace("_Total", arg.filterRows.toString());
      menu = _Str.format(
  ...
  _getPageArg(pageStr) {
    ...
    return {
      ...
      filterRows: (_a = json["filterRows"]) != null ? _a : -1
    };
  }
  find(json = {}, page) {
    const arg = this.pageArg;
    ...
    let url = this.action + "?page=" + arg.pageNo + "&rows=" + arg.pageRows + "&filter=" + arg.filterRows;
...
export {
  ...
  DragItemDto,
  EditDto,
  EditMany3 as EditMany,
  EditModeEstr,
  EditOne,
  ErrorRowDto,
  FlowLine,
  FlowMany,
  FlowNode2 as FlowNode,
  FlowView2 as FlowView,
  FunEstr,
  IdStrDto,
  IdStrExtDto,
  InputTypeEstr,

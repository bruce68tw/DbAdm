...
export default class _ISelect extends _IBase {
    ...
    static setItemsF(filter: string, items: IdStrDto[] | null, box: JQuery): void {
        const obj = _Obj.getF(filter, box);
        _ISelect.setItemsO(obj, items);
    }
    //by object
    static setItemsO(obj: JQueryN, items: IdStrDto[] | null): void {
        if (_Obj.isEmpty(obj)) return;
     ...
     */
    static changeParent(upFid: string, childFid: string, childId: string,
        action: string, isEdit: boolean): void {

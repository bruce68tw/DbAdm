...
export default class _Json {
    ...
    */
     ...
     */
    static toChartRows(json: Json, cols: string[]): { Id: string, Num: any }[] {
     ...
     * return {json} new json data
     */
    static copy(from: Json, to: Json): Json {
        const target = to || {};
    ...
    //convert url to json 
    static urlToJson(url: string): Json {
        if (url.indexOf('?') > -1) {
            url = url.split('?')[1];
        }
    ...
    //find jarray
    //return array index
    static findIndex(rows: Json[] | null, fid: string, value: any): number {
        if (rows == null)
            return -1;
